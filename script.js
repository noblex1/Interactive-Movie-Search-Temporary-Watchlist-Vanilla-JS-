...
function createMovieCard(movie) {
	...
	card.innerHTML = `
		...
		<div style="padding:16px">
			<h3 style="margin:0 0 8px 0;font-size:18px;font-weight:700">${movie.Title}</h3>
			<p style="margin:0 0 12px 0;color:#95a5a6;font-size:14px">${movie.Year} • ${movie.Type}</p>
			<div style="margin-top:12px;display:flex;gap:8px">
				<button class="details-btn">Show Details</button>
				<button class="watch-btn">Add to Watchlist</button>
			</div>
		</div>
		<div class="extra-details" style="padding:16px;display:none;border-top:1px solid rgba(0,0,0,0.06);font-size:14px"></div>
	`;
		...
		fetch(`https://www.omdbapi.com/?apikey=${API_KEY}&i=${movie.imdbID}&plot=short`)
			...
			.then(d => {
				...
				if (d && d.Response !== 'False') {
					extraDiv.innerHTML = `
					<p style="margin:8px 0"><strong>Plot:</strong> ${d.Plot || 'N/A'}</p>
					<p style="margin:8px 0"><strong>Actors:</strong> ${d.Actors || 'N/A'}</p>
					<p style="margin:8px 0"><strong>IMDB Rating:</strong> ${d.imdbRating || 'N/A'}</p>
					`;
...
function renderWatchlist() {
  ...
  watchlist.forEach(item => {
    ...
    div.innerHTML = `
      <img src="${poster}" alt="${item.Title} poster" style="width:100%;height:240px;object-fit:cover;background:#ddd" />
      <div style="padding:16px">
        <h4 style="margin:0 0 12px 0;font-size:16px;font-weight:700">${item.Title}</h4>
